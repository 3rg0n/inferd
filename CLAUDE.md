...
**v0.6.1 (GA).** v0.1 through v0.5 have shipped; the v0.6 line is the current GA line. Seven crates ship: `inferd-proto`, `inferd-engine`, `inferd-daemon`, `inferd-client`, `inferd-openai-wire` (shared OpenAI wire types for the HTTP bridge + outbound adapter), `inferd-http` (a separate, user-launched OpenAI-compat bridge process), and `inferdctl` (the CLI). The IPC wire is frozen per [ADR 0021](docs/adr/0021-unified-v2-wire-length-prefixed-blob-framing.md) (generation) / [ADR 0017](docs/adr/0017-embeddings-on-a-third-socket.md) (embeddings) and did **not** change in v0.6 — the daemon contract is stable.
...
What v0.6.1 adds over v0.6.0: a post-GA correctness/security review sweep, no wire change (`wire_version` unmoved, v0.6.0 clients interoperate). Two THREAT_MODEL fixes — **F-1** per-request attachment bounds (`MAX_ATTACHMENTS_PER_REQUEST` 32 / `MAX_ATTACHMENT_BYTES_PER_REQUEST` 128 MiB in `inferd-proto::v2::attachment`; the 64 MiB cap bounds one *frame*, and each declared attachment entitled the sender to one more) and **F-17** bounded response writes (`--write-timeout-secs`, default 60s — writes happen downstream of the admission gate, so a peer that stopped reading held a generation slot forever). Plus two engine fixes: a double `llama_sampler_accept` that advanced stateful chain members twice, and a per-token vocab-sized allocation hoisted out of the grammar loop. The one deliberate behaviour change: a request declaring >32 attachments, or >128 MiB of them, is now rejected where it was previously accepted.
